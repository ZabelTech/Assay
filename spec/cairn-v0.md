...
The candidate-shareable URL forms look like:
...
How servers extract the token from a request and how clients carry it on subsequent requests is defined in §9.1.1.
...
### 9.1.1 Token transport

The URL is the **sharing format**; the `Authorization` header is the **wire format**. A candidate hands a single self-contained URL to a recipient; the recipient's MCP client speaks to the server using the same `Authorization: Bearer …` pattern it would use for any other authenticated MCP endpoint. The server accepts both so candidates and standard MCP clients meet in the middle without either side compromising.

#### Server-side acceptance

Conforming servers MUST accept the token on every authenticated request in any of the following equivalent forms:

|Form  |Where the token lives                          |Example                                          |
|------|-----------------------------------------------|-------------------------------------------------|
|Header|`Authorization: Bearer <token>` HTTP header    |`Authorization: Bearer abc123…`                  |
|Query |`?t=<token>` on the endpoint URL               |`https://alice.career/mcp?t=abc123…`             |
|Path  |`/t/<token>` segment immediately after the path|`https://alice.career/mcp/t/abc123…`             |

All three forms are equivalent at the access-control layer. When more than one form is present on the same request, every form MUST carry the same token value; if any disagree, the server MUST reject the request with HTTP 400. This prevents a forwarded URL from being silently overridden by a different header value, or vice versa.

The server MUST NOT include the token in response bodies, error messages, redirect targets, or any other field that may be logged downstream of the access-control check.

#### Client-side handling

Conforming clients SHOULD, on first encountering a tokenized URL, perform the following:

1. Extract the token from the `?t=` query parameter or `/t/` path segment.
2. Compute the canonical endpoint URL by removing the token portion.
3. Store the token in the client's existing credential storage (the same place it would store an OAuth bearer token for any other MCP server).
4. Send subsequent requests, including the initial MCP `initialize` handshake, to the canonical endpoint with `Authorization: Bearer <token>`.

Clients that send the token in the URL form on every request remain conformant — the server accepts that — but the header form is preferred for credential hygiene: URLs appear in browser history, intermediate proxies, copy-paste, and Referer headers; the `Authorization` header does not.

#### Transport scope

Servers exposing HTTP and WebSocket MCP transports MUST support all three forms. For WebSocket transports, the URL forms apply to the upgrade request URL and the header form applies to the upgrade request headers; the same equivalence rule holds. Other MCP transports are out of scope for v0.

#### Relationship to MCP authentication conventions

Cairn's bearer-token shape is identical to the bearer-token form used elsewhere in the MCP ecosystem (including by servers that issue tokens via OAuth 2.1). A Cairn server MAY additionally publish `.well-known/oauth-protected-resource` advertising that it accepts bearer tokens; v0 does not require this and does not specify an OAuth authorization-server flow (issuance is candidate-side, §9.2). Off-the-shelf MCP clients that expect to acquire tokens via OAuth and present them as bearer credentials interoperate with Cairn at the wire-protocol level even though Cairn's issuance flow is different.
